...
%observation period, and the target latitude, and display the various observation
%times that various eccentricities and inclinations would work
clc;clf;clear

%Inputs
a=0.48;
TargetLat=60;
LightnessFactor=0.1;%In case you want to simulate the effect of pointing the sail at the sun. if not, just set to 0
plotOptions=[1,1];
myEccStep=0.01;
myIncStep=0.01;
TargetObservationPeriod=40; %days
ConeVerticies=100;
ShowCone=1;
DisplayTable=1;
...
Period=2*pi*sqrt(a^3/uSun)/24/3600; %Theta Vector
ecc=0:myEccStep:0.9; %eccentricity Vector
myIncValues=(0:myIncStep:pi/2); %Inclination Values
myBest3DXs = zeros(length(myIncValues),length(Theta));
myBest3DYs = zeros(length(myIncValues),length(Theta));
myBest3DZs = zeros(length(myIncValues),length(Theta));
myBestEccs = zeros(length(myIncValues),1);

for i=1:length(myIncValues)
    Inclination=myIncValues(i);
    ...
    if ~sum(sum(BinaryMatrix))
        ...
        ValidIndices=find(tDays>TargetObservationPeriod);
        if sum(ValidIndices)
            myBest3DXs(i,:) = my3DX(ValidIndices(1),:);
            myBest3DYs(i,:) = my3DY(ValidIndices(1),:);
            myBest3DZs(i,:) = my3DZ(ValidIndices(1),:);
            myBestEccs(i) = ecc(ValidIndices(1));
        end
...
if plotOptions(1)
    clf(1)
    figure (1)
    plot(myIncValues*180/pi, myBestEccs); grid on;
    xlabel("inclination"); ylabel("min eccentricity required")
end
if plotOptions(2)
    Temp=sum(plotOptions(1:2));
    clf(Temp)
    figure (Temp)
    for J=1:length(myIncValues)
        plot3( myBest3DXs(J,:),myBest3DYs(J,:),myBest3DZs(J,:)); hold on;
    end
    %Plot Sun
    [SunX SunY SunZ]=sphere(10); SunX=SunX.*rSun; SunY=SunY.*rSun; SunZ=SunZ.*rSun;
    surf(SunX, SunY, SunZ);
    
    if ShowCone
        %Plot Target Cone:
        rConeVec = linspace(0,max(max(myBest3DZs))/tan(TargetLat),ConeVerticies) ;
...
%TRUNCATE
GoodVals=find(myBestEccs~=0);
myBest3DXs=myBest3DXs(GoodVals,:); myBest3DYs=myBest3DYs(GoodVals,:); myBest3DZs=myBest3DZs(GoodVals,:);
myR=sqrt(myBest3DXs.^2 + myBest3DYs.^2 + myBest3DZs.^2);
myBestEccs=myBestEccs(GoodVals,:); myIncValues=myIncValues(GoodVals); 
if DisplayTable
    fprintf("At a semi major axis of %.3f AU an orbit will take %.2f days. In order to reach the %.0f day observation period: \n",a,Period,TargetObservationPeriod)
    eccentricityValues=myBestEccs; inclination=myIncValues'*180/pi; Apogees=max(myR')'; Perigees=min(myR')';
    disp(table(eccentricityValues, inclination, Apogees, Perigees))
end
